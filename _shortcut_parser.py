...
def match_contents(string):
    ...
    return Shortcut(
        ...
        icon='"' + regex.groups()[3] + '"' if regex.groups()[3] else '',
        ShortcutPath=regex.groups()[4],
        LaunchOptions=regex.groups()[5],
        IsHidden=parse_bool(regex.groups()[6]),
        AllowDesktopConfig=parse_bool(regex.groups()[7]),
        AllowOverlay=parse_bool(regex.groups()[8]),
        OpenVR=parse_bool(regex.groups()[9]),
        Devkit=parse_bool(regex.groups()[10]),
