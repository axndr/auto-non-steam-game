...
class User(object):
    ...
    def _user_config_directory(self):
        return os.path.join(
            ...
            # "C:\\Program Files (x86)\\Steam\\userdata\\33228265\\",
            "config"
        )
    ...
    def _load_shortcuts(self):
        ...
        if parsed_shortcuts == None:
            ...
            # print(f"Parsing error on file: %s" % file
            print(f"Parsing error on file: FILE SHOULD GO HERE")
            parsed_shortcuts = []
        return parsed_shortcuts

    def userdata_directory(self, ):
        return os.path.join(
            self.steam.userdata_directory,
            str(self.id32)
        )
